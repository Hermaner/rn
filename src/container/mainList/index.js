...
import { View, TouchableOpacity, BackHandler, TouchableWithoutFeedback, FlatList } from 'react-native';
import PropTypes from 'prop-types';
import { Container, Content, Text, Header, Icon } from 'native-base';
import { connect } from 'react-redux';
import { popRoute, pushRoute } from '../../actions';
import { GoodList, Loading, NoData } from '../../components';
      ...
      <Header style={{ alignItems: 'center', backgroundColor: '#f8f8f8' }}>
        ...
        <TouchableWithoutFeedback onPress={() => { push({ key: 'MainSearcher', params: { type: 'getMainListName' } }); }}>
          <View style={styles.HeaderMain}>
            ...
            <Text style={styles.HeaderMainText}>{name || '输入货品名称'}</Text>
          </View>
        </TouchableWithoutFeedback>
        <TouchableOpacity onPress={() => { push({ key: global.memberId ? 'GoodsScreen' : 'User', params: { type: 'getMainList' } }); }}>
                    ...
                    <View style={[styles.fr, { flexWrap: 'wrap' }]}>
                      {
                        items.specs.map((item, i) => (
                          <TouchableWithoutFeedback key={i} onPress={() => this.specsTab(index, i)}>
                            <View
                              style={[
                                styles.contetnTabView,
                                item.cur &&
                                styles.specsTabBackground]}
                            >
                              <Text
                                style={[
                                  styles.mainText,
                                  item.cur && styles.specsTabColor]}
                              >
                                {item.specName}
                              </Text>
                            </View>
