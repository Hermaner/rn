...
import { GetWithdrawalsOrderService } from '../../api';
...
class Base extends React.Component {
  ...
  _onRefresh = () => {
    this.setState({
      refresh: true,
      currentPage: 1,
    }, () => this.GetWithdrawalsOrderService());
  }
  _reachEnd = () => {
    const { nomore } = this.state;
    if (canEnd && !nomore) {
      canEnd = false;
      this.setState({ loading: true }, () => this.GetWithdrawalsOrderService());
    }
  }
}
Base.propTypes = {
