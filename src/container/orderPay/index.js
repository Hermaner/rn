...
import { Container, Content, Input, Text, CheckBox } from 'native-base';
    ...
    return (
      <View style={styles.pagebody}>
        ...
        <View style={styles.flexRow}>
          ...
          <Text style={{ fontSize: 14, color: '#FF8711' }}></Text>
        </View>
      </View>
    );
  }
      ...
      <View style={styles.card}>
          ...
          cardList.map((item, index) => (
            <View style={[styles.cardBox, item.isBorder ? styles.border : '']} key={index}>
              <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center' }}>
                <Image style={styles.zhiImg} source={item.img} />
                <View style={{ flex: 1 }}>
        ...
        <View style={styles.btnBigBox}>
          <View style={styles.btnBox}>
            <Text style={{ fontSize: 14, color: '#fff', textAlign: 'center' }}>支付￥48.00</Text>
          </View>
        </View>
      </View>
    );
  ...
  _renderFooter() {
    return (
      <View style={styles.footer}>
        ...
        <Text style={{ fontSize: 14, color: '#FF8711' }}>请勿相信先确认收货，再申请退款，有问题请联系平台客服</Text>
      </View>
    );
  }
  render() {
